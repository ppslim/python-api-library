...
class Comment(KayakoObject):
	...
	@classmethod
	def _parse_comment(cls, comment_tree, knowledgebasearticleid):

		params = dict(
			id=cls._get_int(comment_tree.find('id')),
			knowledgebasearticleid=knowledgebasearticleid,
			creatortype=cls._get_int(comment_tree.find('creatortype')),
			creatorid=cls._get_int(comment_tree.find('creatorid')),
			fullname=cls._get_string(comment_tree.find('fullname')),
			email=cls._get_string(comment_tree.find('email')),
			ipaddress=cls._get_string(comment_tree.find('ipaddress')),
			dateline=cls._get_date(comment_tree.find('dateline')),
			parentcommentid=cls._get_int(comment_tree.find('parentcommentid')),
			commentstatus=cls._get_int(comment_tree.find('commentstatus')),
			useragent=cls._get_string(comment_tree.find('useragent')),
			referrer=cls._get_string(comment_tree.find('referrer')),
			parenturl=cls._get_string(comment_tree.find('parenturl')),
			contents=cls._get_string(comment_tree.find('contents')),
		)
		return params

	def _update_from_response(self, comment_tree):

		for int_attr in ['id', 'knowledgebasearticleid', 'creatortype', 'creatorid', 'commentstatus', 'useragent', 'parentcommentid']:
			attr = comment_tree.get(int_attr)
			if attr is not None:
		...
		for str_attr in ['contents', 'fullname', 'email', 'ipaddress', 'referrer', 'parenturl']:
			attr = comment_tree.get(str_attr)
			if attr is not None:
				setattr(self, str_attr, attr)

		for date_attr in ['dateline']:
			attr = comment_tree.get(date_attr)
			if attr is not None:
	...
	def get_all(cls, api, knowledgebasearticleid):
		...
		return [Comment(api, **cls._parse_comment(comment_tree, knowledgebasearticleid)) for comment_tree in tree.findall('kbarticlecomment')]
	...
	def get(cls, api, knowledgebasearticleid, id):
		...
		if node is None:
			return None
		params = cls._parse_comment(node, knowledgebasearticleid)
		return Comment(api, **params)
	...
	def __str__(self):
		return '<Comment (%s): %s>' % (self.id, self.contents)
