...
class Article(KayakoObject):
    ...
    __parameters__ = ['id', 'kbarticleid', 'contents', 'contentstext', 'categories', 'creator', 'creatorid', 'author', 'email', 'subject', 'isedited', 'editeddateline', 'editedstaffid', 'views', 'isfeatured', 'allowcomments', 'totalcomments', 'hasattachments', 'attachments', 'dateline', 'articlestatus', 'articlerating', 'ratinghits', 'ratingcount', 'categoryid']
    ...
    @classmethod
    def _parse_article(cls, api, article_tree):
        categories = []
        categories_node = article_tree.find('categories')
        ...
        attachments = []
        attachments_node = article_tree.find('attachments')
        ...
        params = dict(
            id=cls._get_int(article_tree.find('kbarticleid')),
            kbarticleid=id,
            contents=cls._get_string(article_tree.find('contents')),
            contentstext=cls._get_string(article_tree.find('contentext')),
            creator=cls._get_string(article_tree.find('creator')),
            creatorid=cls._get_int(article_tree.find('creatorid')),
            author=cls._get_string(article_tree.find('author')),
            email=cls._get_string(article_tree.find('email')),
            subject=cls._get_string(article_tree.find('subject')),
            isedited=cls._get_boolean(article_tree.find('isedited')),
            categories=categories,
            editeddateline=cls._get_date(article_tree.find('editeddateline')),
            editedstaffid=cls._get_int(article_tree.find('editedstaffid')),
            views=cls._get_int(article_tree.find('views')),
            isfeatured=cls._get_boolean(article_tree.find('isfeatured')),
            allowcomments=cls._get_boolean(article_tree.find('allowcomments')),
            totalcomments=cls._get_boolean(article_tree.find('totalcomments')),
            hasattachments=cls._get_boolean(article_tree.find('hasattachments')),
            attachments=attachments,
            dateline=cls._get_date(article_tree.find('dateline')),
            articlestatus=cls._get_boolean(article_tree.find('articlestatus')),
            articlerating=cls._get_int(article_tree.find('articlerating')),
            ratinghits=cls._get_int(article_tree.find('ratinghits')),
            ratingcount=cls._get_int(article_tree.find('ratingcount')),
        )
        return params

    def _update_from_response(self, article_tree):
        categories_node = article_tree.find('categories')
        if categories_node is not None:
            categories = []
            for id_node in categories_node.findall('categoryid'):
        ...
        for bool_node in ['isedited', 'isfeatured', 'allowcomments', 'totalcomments', 'hasattachments', 'articlestatus']:
            node = article_tree.find(bool_node)
            if node is not None:
    ...
    def get_all(cls, api, categoryid, count=100, start=0):
        ...
        return [Article(api, **cls._parse_article(api, article_tree)) for article_tree in tree.findall('kbarticle')]

    @classmethod
    def get(cls, api, kbarticleid):
        response = api._request('%s/%s/' % (cls.controller, kbarticleid), 'GET')
        tree = etree.parse(response)
        node = tree.find('kbarticle')
        if node is None:
            return None
        params = cls._parse_article(api, node)
        return Article(api, **params)
    ...
    def add(self):
        ...
        Optional:
            ...
            ratingcount           The rating count for the Article.
        '''
        if self.kbarticleid is not UnsetParameter:
            raise KayakoRequestError('Cannot add a pre-existing %s. Use save instead. (id: %s)' % (self.__class__.__name__, self.kbarticleid))
    ...
    def save(self):
        response = self._save('%s/%s/' % (self.controller, self.kbarticleid))
        tree = etree.parse(response)
        node = tree.find('kbarticle')
        self._update_from_response(node)
    ...
    def __str__(self):
        return '<Article (%s): %s>' % (self.id, self.contents)