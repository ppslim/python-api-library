...
class Attachment(KayakoObject):
	...
	@classmethod
	def _parse_attachment(cls, attachment_tree):

		params = dict(
			id=cls._get_int(attachment_tree.find('id')),
			kbarticleid=cls._get_int(attachment_tree.find('kbarticleid')),
			filename=cls._get_string(attachment_tree.find('filename')),
			filesize=cls._get_int(attachment_tree.find('filesize')),
			filetype=cls._get_string(attachment_tree.find('filetype')),
			contents=cls._get_string(attachment_tree.find('contents')),
			dateline=cls._get_date(attachment_tree.find('dateline')),
		)
		return params

	def _update_from_response(self, attachment_tree):
		for int_node in ['id', 'kbarticleid', 'filesize']:
			node = attachment_tree.find(int_node)
			if node is not None:
				setattr(self, int_node, self._get_int(node, required=False))

		for str_node in ['filename', 'filetype', 'contents']:
			node = attachment_tree.find(str_node)
			if node is not None:
				setattr(self, str_node, self._get_string(node))

		for date_node in ['dateline']:
			node = attachment_tree.find(date_node)
			if node is not None:
	...
	def get_all(cls, api, kbarticleid):
		...
		return [Attachment(api, **cls._parse_attachment(attachment_tree)) for attachment_tree in tree.findall('kbattachment')]
	...
	def get(cls, api, kbarticleid, attachmentid):
		...
		if node is None:
			return None
		params = cls._parse_attachment(node)
		return Attachment(api, **params)
	...
	def __str__(self):
		return '<Attachment (%s): %s>' % (self.id, self.filename)
